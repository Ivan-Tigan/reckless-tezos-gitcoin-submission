...
//        member this.n = memoize <| fun s -> this.GetNode(new NodePath(s)) :?> 'a
        ...
        member this.set_text lts = for (l, t) in lts do match (this.get_node<Node> l).Value with | :? LineEdit as l -> l.Text <- t | :? Label as l -> l.Text <- t | :? RichTextLabel as l -> l.Text <- t | :? Button as l -> l.Text <- t| _ -> failwithf "Failed setting text %A %A" l t 
        ...
        member this.set_texture lts =
            for (l, t) in lts do
                ...
                | _ -> failwithf "Failed setting texture %A %A" l t 
...
//    let aes = new AES("5cd18d9a9ff108d4cc8ad3c410ddf9931862427646538211fac53527f92bf5cb")
    ...
    let skynet_upload_string (s:string) = 
        SiaSkynetClient().UploadFileAsync("token_metadata", MemoryStream(Encoding.UTF8.GetBytes(s))) |>> (fun res -> sprintf "https://siasky.net/%s" res.Skylink) |> Async.AwaitTask
    let skynet_upload_img path =
        SiaSkynetClient().UploadFileAsync("token_texture", File.OpenRead path) |>> (fun r -> sprintf "https://siasky.net/%s" r.Skylink) |> Async.AwaitTask
    let skynet_upload_token_metadata name symbol decimals prefer_symbol thumbnail (textures:string list) =
        async{
